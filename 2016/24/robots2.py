...
def distance(s):
    if s.numbers_left and 0 not in s.numbers_left:
        return len(s.numbers_left) + 1
    else:
...
def moves(s):
    for x, y in manhattan_neighbours(s.position):
        c = ducts[y][x]
        if c != '#':
            if c.isdigit() and int(c) in s.numbers_left:
                new_dests = s.numbers_left - {int(c)}
                if not new_dests and c != '0':
                    yield new_state(Point(x, y), frozenset({0}))
                else:
                    yield new_state(Point(x, y), new_dests)
            else:
