...
def maxstar(start_state, move_function, distance_function):
        ...
        for s in move_function(best):
            ...
            if (s not in path_lengths or path_lengths[s] < pl):
                ...
                if d != 0:
                    ...
                    all_paths.append(s)
    if all_paths:
        best = max(all_paths, key=lambda s: path_lengths[s])
        return path_lengths[best]
    else:
        return False
...
start_state = ((0, 0), '')
result = maxstar(start_state, valid_moves, distance)
if result:
    print(result)
